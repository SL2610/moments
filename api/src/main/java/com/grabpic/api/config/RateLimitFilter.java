...
public class RateLimitFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        // Photo bytes (signed view URLs, owner uploads) are exempt: a single
        // gallery load fetches hundreds of images and uploads are JWT-gated.
        if (path.startsWith("/api/photos/view") || path.endsWith("/photos/content")) {
            // fall through without consuming a bucket
        }

        else if (path.startsWith("/api/auth/") || path.equals("/api/wedding/join")) {
            if (!tryConsume("rl:" + ip + ":auth-endpoints", 10, 10)) {
                reject(httpRes, "Too many attempts. Please wait a moment.");
                return;
            }
        }

        else if (path.contains("/guest/search-results")) {
